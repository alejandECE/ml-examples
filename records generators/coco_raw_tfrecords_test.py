...
if __name__ == '__main__':
  ...
  print('Number of images: {}'.format(dataset.cardinality()))
  # Plots nine images
  fig, axes = plt.subplots(3, 3)
  fig.set_tight_layout(tight=0.1)
  axes = axes.ravel()
  # Reads info
  for i, (image, image_id, bboxes) in dataset.take(9).enumerate():
    # Plots corresponding image
    image_height, image_width, image_depth = image.shape
    axes[i].imshow(image.numpy())
    # Plots bounding boxes
    for row in bboxes:
      box = coco.BBox(*row.numpy())
      ...
      axes[i].axis('off')
      axes[i].set_title(image_id.numpy())
  plt.show()
