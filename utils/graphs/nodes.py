...
from utils.graphs.core import needs_backprop
...
class softmax_mce_node(Operation):
  ...
  def backward(self):
    ...
    if needs_backprop(self._inputs[0]):
      upstream = (-(indicator - self.h_node.value).reshape(-1, 1)).dot(self.gradient)
      self._inputs[0].upgradient(upstream)
...
class bias_node(Operation):
  ...
  def forward(self):
    super().forward()
    r = self._inputs[0].value
    b = self._inputs[1].value
    self.value = r + b
  ...
  def backward(self):
    ...
    if needs_backprop(self._inputs[0]):
      ...
      self._inputs[0].upgradient(upstream.copy())
    # Gradient with respect to b
    if needs_backprop(self._inputs[1]):
      upstream = (np.ones((1, m)).dot(upstream.reshape((m, -1)))).T
      self._inputs[1].upgradient(upstream)
...
class linear_node(Operation):
  ...
  def forward(self):
    super().forward()
    X = self._inputs[0].value
    w = self._inputs[1].value
    self.value = X.dot(w)
...
class relu_node(Operation):
  ...
  def backward(self):
    # Gradient with respect to z
    if needs_backprop(self._inputs[0]):
      z = self._inputs[0].value.reshape(-1, 1)
...
class bce_node(Operation):
  ...
  def backward(self):
    ...
    self._inputs[0].gradient = upstream
    # Gradient with respect to y
...
class diff_node(Operation):
  ...
  def backward(self):
    ...
    self._inputs[0].gradient = upstream
    # Gradient with respect to b
    self._inputs[1].gradient = -upstream
